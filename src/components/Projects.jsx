...
const Projects = () => {
  ...
  return (
    <div className='container mx-auto py-4 px-6 md:px-20 lg:px-32 my-20 w-full overflow-hidden' id='Projects'>
      ...
      <div className='overflow-hidden'>
        <div className='flex gap-8 transition-transform during-500 ease-in-out' style={{transform: `translateX(-${(currentIndex * 100) / (cardsToShow)}%)`}}>
            {projectsData.map((project, index)=>(
                <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>
                    <img src={project.image} alt={project.title} className='w-full h-auto mb-14'/>
                    <div className='absolute left-0 right-0 bottom-5 flex justify-center'>
                        <div className='inline-block bg-white w-3/4 px-4 py-2 shadow-md'>
                            <h2 className=' flex flex-row text-l font-semibold text-gray-800'>{project.title}</h2>
